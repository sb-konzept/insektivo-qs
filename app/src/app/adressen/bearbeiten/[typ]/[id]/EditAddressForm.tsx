...
type Address = {
  ...
  certificationSystem?: string | null
  qsId?: string | null
  locationNr?: string | null
...
export default function EditAddressForm({ addressType, address }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: address.phone || '',
    email: address.email || '',
    productionType: address.productionType || '',
    certificationSystem: address.certificationSystem || '',
    qsId: address.qsId || '',
    locationNr: address.locationNr || '',
    vehicleNr: address.vehicleNr || '',
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = addressType === 'lieferanten' 
        ? `/api/suppliers/${address.id}` 
        : addressType === 'kunden' 
        ? `/api/customers/${address.id}` 
        : `/api/carriers/${address.id}`
  ...
  const handleDelete = async () => {
    if (!confirm(`${address.name} wirklich löschen?`)) return
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-6 space-y-6">
      ...
      <div className="flex gap-3 pt-4">
        ...
        <Link
          href="/adressen"
          className="px-4 py-2 text-center border border-gray-300 text-[#4a4a49] rounded-lg hover:bg-gray-50 transition"
        >
          Abbrechen
        </Link>
        <button
          ...
          className="px-6 py-2 bg-[#3c7460] text-white rounded-lg hover:bg-[#2d5a4a] transition disabled:opacity-50"
        >
