...
async function getData() {
  const [boxes, suppliers, customers, carriers] = await Promise.all([
    prisma.box.findMany({ orderBy: { code: 'asc' } }),
    prisma.supplier.findMany({ orderBy: { name: 'asc' } }),
    prisma.customer.findMany({ orderBy: { name: 'asc' } }),
    prisma.carrier.findMany({ orderBy: { name: 'asc' } }),
  ])
  return { boxes, suppliers, customers, carriers }
}
      ...
      <header className="bg-white shadow-sm border-b border-[#3c7460]/20">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <div className="border-l border-[#3c7460]/30 pl-4">
            <h1 className="text-lg font-bold text-[#3c7460]">Buchung erfassen</h1>
            <p className="text-xs text-[#4a4a49]/70">Kistenbewegung dokumentieren</p>
          </div>
        </div>
      </header>
